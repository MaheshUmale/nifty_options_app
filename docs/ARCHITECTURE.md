...
```
┌────────────────┐      ┌────────────────┐      ┌────────────────┐
│  Upstox WS V3  │      │ Mock Generator │      │ SQLite Replay  │
│  (Protobuf)    │      │ (Simulation)   │      │ (Historical)   │
└────────┬───────┘      └────────┬───────┘      └────────┬───────┘
         │                       │                       │
         ▼                       ▼                       ▼
   ┌───────────────────────────────────┐
...
In **Replay Mode**, this layer is bypassed by the `replay_generator` in `app.py`, which injects historical ticks directly from SQLite into the normalizer.

### 2. Instrument Mapping Layer
Maintains a mapping between Upstox `instrument_keys` and human-readable `tradingsymbols`. This ensures that signals generated by the engine carry the exact identifiers required by the Upstox Orders API.

### 3. Analytical Storage (`database.py`)
