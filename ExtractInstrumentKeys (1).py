...
def get_upstox_instruments(symbols=["NIFTY", "BANKNIFTY"], spot_prices={"NIFTY": 0, "BANKNIFTY": 0}):
    ...
    for symbol in symbols:
        ...
        # Slice range: Index - 13 to Index + 13 (Total 27 strikes)
        start_idx = max(0, atm_index - 13)
        end_idx = min(len(unique_strikes), atm_index + 14)
...
def getNiftyAndBNFnOKeys():
    ...
    try:
        ...
        # print(data)
        # 2. Generate a date string in IST
        ...
        # Build a mapping of key -> trading_symbol for better dashboard population
        key_symbol_map = {}
        for symbol in ["NIFTY", "BANKNIFTY"]:
            if symbol in data:
                for opt in data[symbol]["options"]:
                    key_symbol_map[opt["ce"]] = opt["ce_trading_symbol"]
                    key_symbol_map[opt["pe"]] = opt["pe_trading_symbol"]

        print(f"Extracted {len(ALL_FNO)} F&O keys.")
        return ALL_FNO, key_symbol_map
    except ApiException as e:
        print("Exception when calling MarketQuoteV3Api->get_ltp: %s\n" % e)
        return [], {}
    except Exception as e:
        print("Error in getNiftyAndBNFnOKeys: %s\n" % e)
        return [], {}